...
data {
  ...
  matrix[n_site, n_site] M;                       // exp(-downstream distance) matrix for sites (for spatial autocorrelation)
}
parameters {
...
transformed parameters {
  ...
  matrix[n_site, n_site] W = pow( M, lambda );  // exp(-lambda * M);  if M is raw distances
  
  // // Setting the diagonal to zero to avoid a site being its own neighbor
  // Done in data preparation now that M is inversely weighted at that stage
  // for (k in 1:n_site){
...
// See Neal's Funnel: https://mc-stan.org/docs/stan-users-guide/reparameterization.html
  ...
  for (i in 1:n_obs){
    for (j in 1:n_taxa){
      ...
      mu[i,j] = a_taxon[j] + u[i,] * gamma[,j] + a_site[site[i],j] + epsilon[i,j] + delta[site[i],j];
    }
  }
}
model {
  ...
  mu_gamma ~ normal(0,2); // std_normal();                // mean of gamma hyperdistribution
  to_vector(gamma_raw) ~ normal(0,2); //  ~ std_normal();    // raw gamma parameters
