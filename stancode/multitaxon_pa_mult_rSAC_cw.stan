...
data {
  ...
  matrix[n_site, n_site] M_euc;                   // Downstream distance matrix for sites (for spatial autocorrelation)
  matrix[n_site, n_site] M_ds;                    // Euclidean distance matrix for sites (for spatial autocorrelation)
}
parameters {
...
//  real<lower=0> sigma_tau;                        // Hyperprior for scale parameter of tau
  ...
  vector[n_taxa] euc_raw_delta;                    // Raw Euclidean spatially autocorrelated error term
  vector[n_site] mu_euc_delta;                    // Euclidean Spatially autocorrelated mean
  real<lower=0> euc_lamda;                       // Euclidean Spatial exponential decay rate parameter

  vector[n_taxa] ds_raw_delta;                    // Raw dowstream river distance spatially autocorrelated error term
  vector[n_site] mu_ds_delta;                 // Dowstream river distance Spatially autocorrelated mean
  real<lower=0> ds_lamda;                     // Dowstream river distance Spatial exponential decay rate parameter
...
transformed parameters {
  ...
  matrix[n_site, n_taxa] euc_delta;           // Euclidean Spatially autocorrelated error term
  matrix[n_site, n_taxa] ds_delta;            // Dowstream river distance Spatially autocorrelated error term
  
  // LKJ correlation matrix density
  matrix[n_pred, n_pred] L_Sigma = diag_pre_multiply(tau, L_Omega);
  
  // Inverse distance-weighted matrix
  matrix[n_site, n_site] euc_W = exp(-euc_lamda * M_euc);
  matrix[n_site, n_site] ds_W = exp(-ds_lamda * M_ds);
  
  // Setting the diagonal to zero to avoid a site being its own neighbor
  for (k in 1:n_site){
    euc_W[k,k] = 0; // Zero diagonal
    ds_W[k,k] = 0; // Zero diagonal
  }
  ...
  for (j in 1:n_taxa){
    ...
    euc_delta[,j] = mu_euc_delta + euc_raw_delta[j]; // Euclidean patially autocorrelated error term
    ds_delta[,j] = mu_ds_delta + ds_raw_delta[j]; // River distance downstream patially autocorrelated error term
  }
...
// See Neal's Funnel: https://mc-stan.org/docs/stan-users-guide/reparameterization.html
  ...
  for (i in 1:n_obs){
    for (j in 1:n_taxa){
      ...
      mu[i,j] = a_taxon[j] + u[i,] * gamma[,j] + a_site[site[i],j] + epsilon[i,j] + 
                euc_delta[site[i],j] + ds_delta[site[i],j];
    }
  }
}
model {
...
//  L_Omega ~ lkj_corr_cholesky(sigma_Omega);
  ...
  euc_lamda ~ std_normal();  // spatial exponential decay rate parameter
  for (k in 1:n_site){
    // Spatially autocorrelated mean
      mu_euc_delta[k] ~ normal(euc_W[k,] * mu_euc_delta, 1); // Recursive priors can be solved by HMC
  }
  to_vector(euc_raw_delta) ~ std_normal();  // raw spatially autocorrelated error term
  to_vector(ds_raw_delta) ~ std_normal();  // raw spatially autocorrelated error term
