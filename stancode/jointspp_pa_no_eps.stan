...
transformed data {
  // Identity matrix matching species dimensions [n_taxa x n_taxa] - for L_phylo calculation
    matrix[n_taxa, n_taxa] I_taxa;
    I_taxa = identity_matrix(n_taxa); 
}
...
parameters {
  ...
  // strength of phylogenetic influence on betas
...
transformed parameters {
  ...
  lambda = lambda_raw * sigma_lambda;
  {
    ...
    L_phylo_mixed = cholesky_decompose(rho_phylo * phylo_cor + (1.0 - rho_phylo) * I_taxa);
...
model {
  ...
  // by declaring lambda, z_orthogonal and mu inside these braces, they are not saved in the model fit.
  {
